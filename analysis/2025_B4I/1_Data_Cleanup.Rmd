...
IA_phenotypes <- IA_phenotypes %>%
  group_by(plotNumber) %>%
  mutate(
    across(c(Oat_Height_1, Oat_Height_2, Oat_Height_3, Pea_Height_1, Pea_Height_2, Pea_Height_3, Oat_establishment, Pea_establishment),
           ~ ifelse(
             observationLevel == "plot",
             mean(.x[observationLevel == "subplot"], na.rm = TRUE),
             .x
           ))
  ) %>%
  ungroup()
...
```{r}
subplot_data <- multi_location_data %>%
  filter(observationLevel == "subplot")
plot_data <- multi_location_data %>%
  filter(observationLevel == "plot")
```

```{r}
#write_csv(plot_data, 'data/Intermediate/B4I_2025_multi_location_plotdata.csv')
#write_csv(subplot_data, 'data/Intermediate/B4I_2025_multi_location_subplotdata.csv')
```
